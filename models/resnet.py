...
class ResNetS12TinyImage(nn.Module):
    def __init__(self, block, num_blocks, num_classes=200):
        super(ResNetS12TinyImage, self).__init__()
        self.in_planes = 64
    ...
    def forward(self, x):
        out = F.relu(self.bn1(self.conv1(x)))
        print('shape after conv1 : '.format(out.shape))
        out = self.layer1(out)
        print('shape after layer1 : '.format(out.shape))
        
        out = self.layer2(out)
        print('shape after layer2 : '.format(out.shape))
        out = self.layer3(out)
        print('shape after layer3 : '.format(out.shape))
        out = self.layer4(out)
        print('shape after layer4 : '.format(out.shape))
        #out = F.avg_pool2d(out, 4)
        #print('shape after avg_pool2 : '.format(out.shape))
        out = self.gap(out)
        print('shape after GAP : '.format(out.shape))
        return out


def ResNet18():
    #return ResNet(BasicBlock, [2,2,2,2])
    return ResNetS12TinyImage(BasicBlock, [2,2,2,2])
